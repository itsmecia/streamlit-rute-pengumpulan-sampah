...
for key, label in menu_items.items():
    ...
    # Style tombol aktif dan tidak aktif
    button_style = f"""
    ...
    # Render tombol (klik berfungsi)
    clicked = st.sidebar.button(label, key=f"btn_{key}")

    # Gaya CSS untuk tiap tombol
    st.markdown(
        f"""
        <style>
    ...
    # Jika tombol diklik, update state
    if clicked:
