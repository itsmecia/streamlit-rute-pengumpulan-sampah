...
st.title("Sistem Analisis Rute & Pengumpulan Sampah Kota")
...
if mode == "Dashboard Data":
    ...
    if penuh.empty:
        ...
        if not rekom_df.empty:
            st.write(f"- Rata-rata jarak TPS penuh → TPA terdekat: **{rekom_df['Jarak ke TPA (km)'].mean():.2f} km**")
            try:
                ...
                if selected_tps_df.empty:
                    ...
                    for i, point in enumerate(route):
                        ...
                        folium.Marker(
                            ...
                            icon=folium.Icon(color="blue", icon="truck" if i == 0 else "trash", prefix="fa")
                        ).add_to(m)
                        if i < len(route) - 1:
